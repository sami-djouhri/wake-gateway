...
- `app/main.py` is the API: wake, sleep, status, and the reaper that puts idle
...
- `services.example.json` is the registry format: container, upstream, the port
...
- `docker-compose.example.yml` is the arrangement, including a Docker socket
